...
EOF
        ...
        PUBLISH_SCRIPT="$WORKSPACE/scripts/publish_blog_post.py"
        if [ -x "$PUBLISH_SCRIPT" ]; then
            log "📰 Publishing blog HTML + updating site index..."
            if python3 "$PUBLISH_SCRIPT" "$BLOG_FILE" >/tmp/publish_blog.log 2>&1; then
                log "   ↳ Blog ready for GitHub Pages"
            else
                log "   ⚠️ Blog publish helper failed (see /tmp/publish_blog.log)"
            fi
        else
            log "⚠️ Blog publish helper script missing; skipping website update."
        fi
